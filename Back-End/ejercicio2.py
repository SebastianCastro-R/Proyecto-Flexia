...
def mano_cerrada(hand_landmarks):
    """Ejercicio 2: Detectar mano completamente cerrada (puño)"""
    dedos = [8, 12, 16, 20]  # puntas de los dedos
    nudillos = [6, 10, 14, 18]  # nudillos base
    
    # Verificar que los 4 dedos estén flexionados
    dedos_flexionados = sum(hand_landmarks.landmark[d].y > hand_landmarks.landmark[n].y 
...
print("Instrucción: Cierra la mano completamente (PUÑO)")
    ...
    while True:
        ...
        if estado_suavizado and not ultimo_estado:
            ...
            try:
                ...
                conn.sendall(struct.pack(">L", len(mensaje)) + mensaje)
                print("🔄 Ejercicio 2 no completado")
            except (BrokenPipeError, ConnectionResetError):
                print("Conexión con Java perdida (status).")
                break
